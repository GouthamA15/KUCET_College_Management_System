...
> ⚠️ **CANONICAL STORAGE KEY CONTRACT**: Database tables MUST ONLY store canonical relative storage keys representing the folder path and randomized UUID filename (e.g. `clerks/pfp/<uuid>.<ext>` or `students/pfp/<uuid>.<ext>`). Database tables MUST NEVER store full domain URLs, bucket endpoints, or cloud vendor prefixes.
>
> 🔄 **LEGACY COMPATIBILITY**: Legacy records stored with historical `kucet/` prefixes (e.g. `kucet/students/pfp/<uuid>.<ext>`) continue to be supported seamlessly by `getAssetUrl()` and `StorageProvider.getUrl()`.
...
```javascript
{
  path: 'clerks/pfp/7a59662b.webp',      // Canonical relative key stored in DB
...
1. **Provider Agnostic Resolution**: `StorageProvider` resolves the public URL (`getUrl()`) regardless of whether the key is stored as `clerks/pfp/...` or `kucet/clerks/pfp/...`.
