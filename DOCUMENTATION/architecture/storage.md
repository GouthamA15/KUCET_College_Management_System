...
## 🔒 Storage Key Invariant & Canonical Relative Keys Rule

To maintain absolute environment independence and prevent database lock-in, the KUCET CMS architecture enforces the **Canonical Storage Key Rule**:

> ⚠️ **CANONICAL STORAGE KEY CONTRACT**: Database tables MUST ONLY store canonical relative storage keys representing the folder path and randomized UUID filename (e.g. `staff/pfp/<uuid>.<ext>` or `students/pfp/<uuid>.<ext>`). Database tables MUST NEVER store full domain URLs, bucket endpoints, or cloud vendor prefixes.
>
> 🔄 **LEGACY COMPATIBILITY**: Legacy records stored with historical `kucet/` prefixes (e.g. `kucet/students/pfp/<uuid>.<ext>` or `kucet/clerks/pfp/<uuid>.<ext>`) continue to be supported seamlessly by `getAssetUrl()` and `StorageProvider.getUrl()`.

### Canonical Key Examples
- Student Profile Photo: `students/pfp/b3f96f9f4d51487fb2d69fce.webp` (Legacy: `kucet/students/pfp/...`)
- Staff Profile Photo: `staff/pfp/7a59662b.webp` (Legacy: `kucet/clerks/pfp/...`)
- Staff Signature: `staff/signatures/91ec983a.webp`
- Verification Document: `requests/proofs/71a9e1c8ab7d4b6d8d4e7e4a.pdf`
- Issued Certificate PDF: `certificates/bonafide/2026/tc_998124.pdf`

### Provider Responsibilities & Standardized Output
The `StorageProvider` is responsible for generating public URLs based on the active backend.
`StorageProvider.upload()` returns a standardized `StorageResult` object:
```javascript
{
  path: 'staff/pfp/7a59662b.webp',        // Canonical relative key stored in DB
  url: 'https://res.cloudinary.com/...',  // Public URL generated by provider
  filename: '7a59662b.webp',             // Cryptographic randomized filename
  mimeType: 'image/webp',                // MIME type of asset
  provider: 'cloudinary',                // Active provider name ('cloudinary' | 'local' | 's3')
  size: 1024,                            // File size in bytes
  version: 1778497250                    // Optional Cloudinary version timestamp
}
```

### Why Database Values Do Not Require Prefix Forcing
1. **Provider Agnostic Resolution**: `StorageProvider` resolves the public URL (`getUrl()`) regardless of whether the key is stored as `staff/pfp/...` or `kucet/staff/pfp/...`.
2. **Zero Database Migrations**: Switching from Cloudinary to Cloudflare R2 or local disk storage requires **zero SQL update scripts**.
3. **Dynamic CDN Optimization**: Image delivery formats (e.g., WebP/AVIF auto-conversion `f_auto,q_auto`) are tuned globally in `@/lib/assets` and `StorageProvider.getUrl()` at read-time.

---

## 🧪 CI/CD Test Strategy for Integration & Live Storage Tests

To ensure unit test suites remain fast, deterministic, and self-contained when running in CI environments without cloud credentials:

1. **Automated Credential Discovery**: Live Cloudinary integration tests inspect environment variables (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`).
2. **Conditional Skip in CI**: If credentials are not present, test suites automatically skip live cloud network operations (`it.skipIf(!hasCloudinaryCreds)`), ensuring unit tests pass deterministically on local workstations and CI runners alike.
3. **Contract Verification**: Unit tests verify mock and local storage providers against the `StorageResult` contract without asserting fixed vendor-specific prefixes.
...
| **Can URLs be generated correctly without storing version?** | Yes. Omitting the version segment causes Cloudinary CDN to automatically resolve and serve the latest active version. | DB schemas remain pure, storing relative keys (`kucet/staff/pfp/<uuid>.webp` or `staff/pfp/<uuid>.webp`). |
