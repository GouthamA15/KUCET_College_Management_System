...
- **Nested App Router Route Architecture:** Converted the single flat page into a proper Next.js App Router nested route tree:
  - `/clerk/faculty/attendance` â€” Subject selection list (root page, fully persistent).
  - `/clerk/faculty/attendance/[assignmentId]` â€” Mode selector (manual / qr / gps / history). URL encodes assignment identity.
  - `/clerk/faculty/attendance/[assignmentId]/take/[mode]` â€” Live attendance sheet. Both assignment and mode are URL segments.
  - `/clerk/faculty/attendance/[assignmentId]/history` â€” Attendance history viewer.
...
- **Classification Rule Applied:**
  ...
  - **Search Params** (retained for `?tab=`, `?view=`): Tab/filter-level state in HOD, Admin, and Student pages â€” not refactored (correct as-is per project conventions).
...
- **Official Hosting Budget & Migration Plan (`OFFICIAL_HOSTING_BUDGET_AND_MIGRATION_PLAN.md`):**
  ...
  - **College Decision:** Hostinger KVM 2 VPS (2 vCPU / 8GB RAM / 100GB NVMe / Mumbai DC) â€” Rs.11,839.18/yr (Year 1 with NETWORKCHUCK coupon). Domain: kucet.in (3yr) = Rs.2,122.82. Total Year 1 outlay: Rs.13,962.
  ...
  - Year 2 renewal risk documented: +67% spike â†’ Rs.19,809.84. Calendar reminder strategy in place.
...
- **Critical Bug Fixes (August 2, 2026):**
  - **C1 Fixed:** Removed ghost import `_getNow` from `api-utils.js` (non-exported symbol â€” potential bundler crash).
  - **C2 Fixed:** Replaced raw unsigned `fetch()` PUT in `S3StorageProvider.js` with `@aws-sdk/client-s3` `PutObjectCommand` (SigV4 auth â€” previous code always returned 403).
  ...
  - **M6 Fixed:** Added `GOV` to `students.fee_reimbursement` enum in `schema.js` â€” prevents MySQL constraint error when finalizing GOV-category admission drafts.
...
#### **Session 177: Remote Changes by GouthamA15 â€” Student Management UI/UX Improvements (July 25, 2026)**
...
#### **Session 176: Auth Security Hardening â€” Full Audit & Fix Sprint (July 20, 2026)**
- **User Enumeration Elimination (Critical):**
  - `forgot-password/student` GET: Was returning `404` on missing roll numbers â€” now always returns `200 { is_email_verified: false, has_password_set: false }` regardless of account existence.
  - `forgot-password/student` POST: Was returning `404 "Student not found"` and `403 "not activated"` â€” both consolidated to a single generic `200 "If an account exists..."` message in all branches including the error handler.
  - `forgot-password/clerk` POST: Was returning `404 "Clerk not found"` â€” replaced with the same generic `200` message.
...
- **Timing-Safe Comparisons (High):**
  - `verify-otp`: Plain `===` comparison replaced with `crypto.timingSafeEqual(hashOtp(submitted), storedHash)` â€” eliminates timing oracle on OTP verification.
  - `student/login`: DOB `===` string comparison replaced with `crypto.timingSafeEqual` with 255-byte padded buffers â€” eliminates timing oracle on date-of-birth login.
...
- **409 Enumeration (High):**
  - `public/admission/route.js`: Three distinct `409` messages (`"email already registered"`, `"mobile already in use"`, `"Aadhaar already registered"`) consolidated to one generic `"Please check your details and try again."` â€” prevents PII probing via the admission form.
  - `send-update-email-otp`: Was `"This email is already registered to another student"` â†’ same generic `409`.
- **Rate Limiting (Medium):**
  - `forgot-password/admin`, `forgot-password/clerk`, `forgot-password/student`: **3 requests / 15 min per IP** â€” was completely unprotected.
  - `change-password/student`: **5 requests / 15 min** via tiered device key.
- **bcrypt Cost Factor (Medium):** All `bcrypt.hash()` calls in `reset-password`, `change-password/student`, `change-password/clerk`, `change-password/admin` raised from cost `10` â†’ `12`.
...
- **Reset Token Expiry (Medium):** Extended from 10 minutes â†’ **60 minutes** across all three `forgot-password` routes, matching institutional usability requirements.

#### **Session 175: Remote Changes by GouthamA15 â€” Feature Optimizations (July 19, 2026)**
...
- **Student Settings Cleanup:** Removed legacy `src/app/student/settings/page.js` (10 lines removed) â€” now superseded by the granular security sub-page.
- **Clerk Edit-Profile Deprecation:** Renamed `src/app/clerk/settings/edit-profile/page.js` â†’ `_page.js` to temporarily disable the stale page without deleting it.
- **Admission Requests Panel Refactor:** `src/components/clerk/requests/AdmissionRequestsPanel.js` heavily refactored â€” improved layout, sorting, and filter UX with 86 lines reworked across search, filter chips, and pagination controls.
...
- **Admission Students API Cleanup:** Removed 2 lines from `src/app/api/clerk/admission/students/[rollno]/route.js` â€” minor dead-code removal.
...
#### **Session 167: UI Refactor â€“ HOD Dashboard Separation (July 1, 2026)**
- Removed HOD dashboard links from Faculty dashboard and added a dedicated HOD menu entry in the sidebar.

#### **Session 168: Bulk UI Refactor â€“ Admin & Clerk Pages (July 1, 2026)**
