...
**System Version:** Session 218 - Next.js Security Patch & Service Worker Navigation Trap Resolution
**Last Updated:** September 24, 2026
**Status:** Active Production / Synchronized  
**Test Suite Verification:** 78 test files (665 unit tests) + 23 Playwright tests across 7 spec files — testvanilla and main branches synchronized
...
| `7bf0a99` | Session 213 — PWA True Reload, Container Host Realignment & Zero-Downtime Swap | 1. **PWA Sticky Reconnect Fix**: Fixed service worker `public/sw.js` same-document navigation no-op (`location.replace('/')`) by introducing `doRestore()` calling unconditional `window.location.reload()`, and bumped `CACHE_VERSION` to `v6`. 2. **Docker Network Alignment**: Replaced `EXPERIMENT_DB_HOST=127.0.0.1` with `EXPERIMENT_DB_HOST=db` inside container network, resolving `ECONNREFUSED`. 3. **Zero-Downtime Deployment**: Separated container build from container swap in `deploy.sh` and `rollback.sh` (`docker compose build` followed by `docker compose up -d --no-deps`), eliminating the 25-second service downtime window. 4. **Permissions & Mode Hardening**: Enforced `chown deployer:users` and `git config core.fileMode false`. |
| `pending` | Session 214 — Production Performance, Session Restoration & Tailscale Optimization | 1. **Proxy 303 Redirect Cookie Loss Fix**: Wrapped all `NextResponse.redirect()` calls in `src/proxy.js` with `withCookies(res)` to forward refreshed access tokens, refresh tokens, and session identifiers, eliminating token reuse revocations and infinite session restoration loops. 2. **Stale Cookie Purging**: Explicitly expired role companion cookies (`staff_logged_in`, `admin_logged_in`) on unauthorized redirects to `/`. 3. **Nginx Header Consolidation**: Suppressed duplicate upstream `Cache-Control` declarations in `DEPLOYMENT_PACKAGE/nginx/nginx.conf` via `proxy_hide_header Cache-Control;`. 4. **Sub-Resource Deduplication**: Guarded redundant fetching in `ScholarshipDashboardClient.js` and child request pages. |
| `e0571a10` / `d61ab7b9` | Session 216 — Service Worker Navigation Redirect Trap Resolution & PWA Resilience | 1. **SW Navigation Redirect Fix**: Fixed browser `TypeError: Failed to execute 'respondWith'` when navigating unauthenticated users (`request.mode === 'navigate'`) by setting `redirect: 'follow'` on fetch requests in `public/sw.js` and bumping to cache `v7`. 2. **Offline Recovery Navigation**: Updated `OfflineClient.js` to navigate unconditionally to `/` via `window.location.replace('/')`, freeing users from false-positive outage screens. 3. **Cron Hygiene**: Removed duplicate cron jobs between `kucet-dev` and `root` users on production host. 4. **IST Time Machine Hours**: Enforced `hourCycle: 'h23'` in `toISTDate` (`src/lib/date.js`) eliminating midnight hour 24 day overflow. |
| `1cd78d9b` / `Session 217` | Session 217 — Finalize Admissions Branch Scope, TiDB Collation Harmonization & Hard 1 MB Media Limit | 1. **Finalize Admissions Scope**: Restricted branch selector on `/staff/admission/finalize` to individual branches (`allowAllBranches={false}`), removing "All Branches" solely from finalization workflows while keeping global selectors intact. 2. **TiDB Collation Harmonization**: Cleaned migration `0021_students_achievements_schema.sql` by removing unsupported MySQL 8.0 `COLLATE=utf8mb4_0900_ai_ci` and `ON UPDATE CASCADE`, adding `CREATE TABLE IF NOT EXISTS` and statement breakpoints. 3. **Automated Multi-Environment Baseline Rule 21**: Added Rule 21 in `src/db/baseline-rules.js` to detect `student_achievements` in existing databases and avoid CI/CD migration runner crashes. 4. **Certificate Hard 1 MB Limit**: Implemented 3-stage progressive client compression (1600px -> 1200px -> 1000px), server-side MIME inspection and byte length guards in `/api/student/achievements` and `/api/student/signature`, and storage invariant enforcement in `cloudinary.js` and `LocalStorageProvider.js`. 5. **Verification**: 78/78 test files passed (665/665 unit tests), 100% migration audit pass (`npm run db:check`). |
| `pending` | Session 218 -- Next.js Security Patch & Service Worker Navigation Trap Resolution | 1. **Next.js Windows RCE Vulnerability**: Terminated locked Next.js dev server on Windows and upgraded `next` to version `16.3.3` to apply the patch for the critical Unauthenticated Remote Code Execution vulnerability (`GHSA-p293-qw3h-jr36`), resolving sudden server connection drops and crashes on startup. 2. **Service Worker Navigation Trap**: Fixed an infinite auth loop (`ERR_TOO_MANY_REDIRECTS`) between `/` and `/student` by modifying `public/sw.js`. The Service Worker now explicitly bypasses interception for the root path `/` and `/api/` on navigation requests, preventing `redirect: 'follow'` from stripping the newly minted `Set-Cookie` headers attached to the `proxy.js` 303 Redirect. |
