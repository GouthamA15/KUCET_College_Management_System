...
| `pending` | Session 215 — Post-Merge CI / Playwright Attendance Routing Regressions Fix | 1. **App Router Deep-Linking Restored**: Recreated `src/app/staff/faculty/attendance/[assignmentId]/take/[mode]/page.js` supporting direct URLs (`take/manual`, `take/qr`, `take/gps`), invalid mode redirection, and browser back button navigation. 2. **Mode Selector Restoration**: Restored `src/app/staff/faculty/attendance/[assignmentId]/page.js` and `AttendanceModeSelector.js` with `<h2>Select Attendance Mode</h2>`, subject headers, and direct mode cards. 3. **Teammate Enhancement Preservation**: Preserved Indian Standard presentation date formatting (`DD-MM-YYYY`) and roll number canonicalization (`canonicalizeRollNo`) in `AttendanceSheet.js` and `MobileAttendanceSheet.js`. 4. **CI ESLint Fix**: Destructured `admissionDrafts` from `useStaff()` in `src/app/staff/admission/requests/page.js` and cleaned unused `LoadingSpinner` in `src/app/student/layout.js`. 5. **Verification**: 100% pass rate across all 23 Playwright tests and 77 Vitest unit test files (640 unit tests). |
