...
- **Production Hardening Sprint — Phase 2: Service Architecture:**
  - **God-Service Decoupling (`41ffecb`):** Decoupled `StudentService.js` by introducing `StudentCertificateService.js` (`src/services/identity/StudentCertificateService.js`) to encapsulate Bonafide, TC, and NOC certificate eligibility rules, maintaining 100% backward compatibility via facade delegation. Covered by Vitest unit tests.
