...
export class StaffRegistrationService {
  ...
  static async submitRegistrationRequest({
    ...
    // 3. Duplicate check in active staff
    const existingStaff = await db.select({ id: staffAccounts.id })
      .from(staffAccounts)
      .where(or(
    ...
    if (mobile) {
      ...
      if (cleanMobile) {
        mobileHash = encrypt(cleanMobile);
      }
    }
    ...
    logger.info({ requestId, email: cleanEmail, employee_id: cleanEmpId, category, branch: validatedBranch }, '[STAFF_REGISTRATION] Pending request submitted');
  ...
  static async approveRequest(requestId, adminId) {
    ...
    const [staffResult] = await db.insert(staffAccounts).values({
      name: request.name,
      email: request.email,
    ...
    logger.info({ requestId, newStaffId, email: request.email, emailSent }, '[STAFF_REGISTRATION] Approved request & sent credentials');
  ...
  static async rejectRequest(requestId, adminId, reason = '') {
    ...
    logger.info({ requestId, email: request.email, emailSent }, '[STAFF_REGISTRATION] Rejected request');
   ...
   */
  static async changePassword(staffId, currentPassword, newPassword) {
    ...
    logger.info({ staffId }, '[STAFF_REGISTRATION] Password changed successfully');
