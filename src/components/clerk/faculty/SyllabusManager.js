...
export default function SyllabusManager({ branch }) {
  ...
  return (
    <div className="animate-in fade-in duration-500">
        ...
        {loading ? (
          ...
          syllabus.map(sub => (
            <div key={sub.subject_code} className="bg-white border border-slate-300 rounded-sm overflow-hidden group hover:border-slate-400 transition-colors shadow-sm">
              <div className="p-5 flex justify-between items-start gap-4 bg-slate-50 border-b border-slate-200">
                <div>
              ...
              <div className="px-5 py-5 space-y-3">
                <div className="text-sm font-bold text-slate-900 uppercase tracking-[0.22em] border border-slate-300 bg-slate-100 px-3 py-2 rounded-sm">
                  Unit Details ({sub.units?.length || 0})
                </div>
                {sub.units?.map(u => {
                  const topics = safeParse(u.topics);
                  return (
                    <div key={u.id} className="bg-slate-50 rounded-sm p-4 border border-slate-300 hover:border-[#0b3578]/25 hover:bg-[#0b3578]/[0.06] transition-colors">
                      <div className="flex justify-between items-center gap-3 mb-2">
                        <h5 className="font-bold text-slate-800 text-[11px] uppercase tracking-wide leading-snug">{u.unit_name}</h5>
                        <button onClick={() => setEditingUnit(u)} className="text-[10px] font-bold text-[#0b3578]/70 hover:text-[#0b3578] uppercase tracking-[0.2em]">
                          Modify Topics
                        </button>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {topics.slice(0, 3).map((t, i) => (
                          <span key={i} className="text-[10px] font-semibold text-slate-700 bg-white border border-slate-200 px-2 py-0.5 rounded-sm line-clamp-1">{t}</span>
                        ))}
                        {topics.length > 3 && <span className="text-[9px] font-medium text-slate-400 px-1">+{topics.length - 3} more</span>}
                      </div>
                    </div>
                  );
                })}
                <button 
                  ...
                  className="w-full py-3 bg-[#0b3578] text-white border border-[#0b3578] rounded-sm font-bold text-[10px] uppercase tracking-[0.22em] hover:bg-[#0b3578]/90 transition-colors"
                >
      ...
      {editingUnit && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[110] flex items-center justify-center px-4 py-10 overflow-y-auto">
          <div className="bg-white border border-slate-200 shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 rounded-sm">
            <div className="bg-[#0b3578] p-4 text-white flex justify-between items-center border-b border-white/10">
              <h3 className="font-bold text-sm uppercase tracking-widest">Modify Unit Content</h3>
              <button onClick={() => setEditingUnit(null)} className="text-white/70 hover:text-white font-bold text-[10px] uppercase tracking-[0.2em]">Close</button>
            </div>
            <form onSubmit={handleSaveUnit} className="p-6 sm:p-8 space-y-5 bg-white">
              <div className="grid grid-cols-4 gap-4">
                <div className="col-span-1">
                  <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-[0.2em] mb-1">Order</label>
                  <input name="order" type="number" defaultValue={editingUnit.unit_order} required className="w-full bg-slate-50 border border-slate-200 rounded-sm p-3 text-sm font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-[#0b3578]/30" />
                </div>
                <div className="col-span-3">
                  <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-[0.2em] mb-1">Unit Title</label>
                  <input name="name" defaultValue={editingUnit.unit_name} required className="w-full bg-slate-50 border border-slate-200 rounded-sm p-3 text-sm font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-[#0b3578]/30" placeholder="e.g. UNIT-I INTRODUCTION" />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-[0.2em] mb-1">Detailed Topics (One per line)</label>
                <textarea 
                  name="topics" 
                  defaultValue={safeParse(editingUnit.topics).join('\n')} 
                  required 
                  rows={10} 
                  className="w-full bg-slate-50 border border-slate-200 rounded-sm p-3 text-sm font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-[#0b3578]/30 resize-none" 
