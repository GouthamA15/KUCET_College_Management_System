      ...
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="relative group max-w-sm w-full hidden sm:block">
          ...
          <input
            ...
            className="w-full bg-white lg:bg-slate-100/50 border border-slate-200 lg:border-none rounded-xl py-2 pl-10 pr-4 text-xs focus:ring-1 focus:ring-[#0b3578]/20 focus:bg-white transition-all outline-none"
          />
        </div>
      </div>
                  ...
                  {notifications.length > 0 ? (
                    notifications.map((n) => (
                      <div key={n.id} className="block p-5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0 relative group">
                        ...
                        <button onClick={handleDismiss} className="absolute top-4 right-4 text-slate-300 hover:text-slate-600 transition-colors p-1">
