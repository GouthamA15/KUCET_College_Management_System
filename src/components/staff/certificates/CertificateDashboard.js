...
export default function CertificateDashboard({ clerkType }) {
  ...
  // Realtime handling for history auto-refresh
  const handleRealtime = useCallback((data) => {
    if (data.type === 'REQUEST_UPDATED' || data.type === 'REQUEST_CREATED') {
      // If we are currently showing history or filtered results, we need to refresh to see the new data
      if (workspaceMode === 'history' || appliedFilters.certificateType.length > 0 || appliedFilters.status.length > 0) {
        fetchRecords();
      }
    }
  }, [fetchRecords, workspaceMode, appliedFilters]);
