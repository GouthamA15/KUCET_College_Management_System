...
export default function FacultyAcademicCalendar({ assignment, selectedDate, onSelectDate, attendanceSectionId }) {
  ...
  const fetchSemesterWindow = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/staff/semesters?academic_year=${encodeURIComponent(academicYear)}&semester=${encodeURIComponent(academicTerm)}`);
  ...
  const fetchCalendar = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/staff/academic-calendar?academic_year=${encodeURIComponent(academicYear)}&semester=${encodeURIComponent(
          academicTerm
