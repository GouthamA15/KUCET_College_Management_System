        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                {activeAssignments.length > 0 ? (
                    <div>
                        <div className="bg-green-50 border border-green-200 text-green-800 rounded p-4 mb-3">
                            <h3 className="font-bold mb-1">HOD Access Active</h3>
                            <div className="grid grid-cols-2 gap-4 text-sm mt-2">
                            <div className="col-span-2"><span className="text-green-600/80 uppercase text-[10px] tracking-wider block font-black">Program(s)</span>{Array.from(new Set(activeAssignments.map(a => a.department_code))).join(', ')}</div>
                            <div><span className="text-green-600/80 uppercase text-[10px] tracking-wider block font-black">Academic Year</span>{activeAssignments[0].academic_year}</div>
                            <div><span className="text-green-600/80 uppercase text-[10px] tracking-wider block font-black">Valid From</span>{formatDate(activeAssignments[0].start_date)}</div>
                    ...
                    <div>
                        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded p-4 mb-3">
                            <h3 className="font-bold mb-1">Request Pending</h3>
                            <p className="text-sm">Your request for HOD access ({Array.from(new Set(pendingRequests.map(p => p.department_code))).join(', ')}, {pendingRequests[0].academic_year}) is pending approval by the Admin.</p>
                        </div>
                    </div>
                ) : expiredAssignments.length > 0 ? (
                    <div>
                        <div className="bg-gray-50 border border-gray-200 text-gray-500 rounded p-4 mb-3">
                            <h3 className="font-bold mb-1">HOD Access Expired</h3>
                            <p className="text-sm">Your previous HOD access for {Array.from(new Set(expiredAssignments.map(a => a.department_code))).join(', ')} ({expiredAssignments[0].academic_year}) expired on {formatDate(expiredAssignments[0].end_date)}.</p>
                        </div>
                    </div>
                ) : (
            ...
            {rejectedRequests.length > 0 && !hasActiveAccess && !hasPendingRequest && (
                <div>
                    ...
                    <div className="bg-red-50 border border-red-200 text-red-800 rounded p-4 mb-3">
                        <h3 className="font-bold mb-1">Rejected</h3>
                        <p className="text-sm">Your request for {Array.from(new Set(rejectedRequests.map(r => r.department_code))).join(', ')} ({rejectedRequests[0].academic_year}) was rejected.</p>
                        {rejectedRequests[0].rejection_reason && <p className="text-sm mt-1">Reason: {rejectedRequests[0].rejection_reason}</p>}
                        <p className="text-xs text-red-600 mt-2">Reviewed on {formatDate(rejectedRequests[0].reviewed_at)}</p>
                    </div>
                </div>
            )}
        </div>
