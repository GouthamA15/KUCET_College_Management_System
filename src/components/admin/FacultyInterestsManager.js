          ...
          {interests.map((interest) => (
            <div key={interest.id} className="bg-white border border-gray-200 p-4 rounded-lg shadow-sm">
              <div className="flex justify-between items-start mb-3 border-b border-gray-100 pb-3">
                <div>
                  <div className="font-bold text-gray-900">{interest.faculty_name}</div>
                  <div className="text-xs text-gray-500">ID: {interest.employee_id}</div>
                </div>
                <span className={`px-2 py-1 inline-flex text-[10px] font-bold uppercase tracking-widest rounded-full ${
                  interest.status === 'APPROVED' ? 'bg-green-100 text-green-800 border border-green-200' :
                  interest.status === 'REJECTED' ? 'bg-red-100 text-red-800 border border-red-200' :
              ...
              <div className="mb-3">
                <div className="text-sm font-bold text-gray-800">{interest.subject_name}</div>
                <div className="text-xs font-mono text-gray-500 mb-1">{interest.subject_code}</div>
                <div className="text-xs text-gray-600 font-medium">
                  {interest.branch} | Sem {interest.semester} | {interest.academic_year}
                </div>
                {interest.allocated_faculty_name && (
                  <div className="mt-2 flex items-center gap-1 text-[10px] text-amber-700 font-bold bg-amber-50 px-2 py-1 rounded border border-amber-200">
                    <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                    </svg>
                    Allocated to: {interest.allocated_faculty_name}
                  </div>
                )}
              </div>
              ...
              {interest.status === 'PENDING' ? (
                <div className="flex gap-2 pt-3 border-t border-gray-100">
                  <button
                    onClick={() => handleAction(interest.id, 'APPROVED')}
                    disabled={processing}
                    className="flex-1 bg-green-600 text-white text-xs font-bold px-3 py-2 rounded hover:bg-green-700 disabled:opacity-50"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => handleAction(interest.id, 'REJECTED')}
                    disabled={processing}
                    className="flex-1 bg-red-600 text-white text-xs font-bold px-3 py-2 rounded hover:bg-red-700 disabled:opacity-50"
                  >
                    Reject
                  </button>
                </div>
