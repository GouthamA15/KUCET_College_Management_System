...
export async function GET(_request) {
  try {
    ...
    try {
      ...
      logger.error({ err: reqErr }, '[HOD_REQUESTS] Error querying faculty HOD requests table');
    ...
    try {
      activeAssignments = await db.select({
      ...
      logger.warn({ err: assignErr }, '[HOD_REQUESTS] Error querying active HOD assignments, continuing with empty assignments');
    }
    ...
    logger.error({ err: error }, 'Error fetching HOD requests:');
