...
import { db } from '@/db';
import { students as studentsTable, staffAcademicAffiliations, academicDepartments, academicPrograms, studentAchievements } from '@/db/schema';
...
export async function GET(request, { params }) {
  try {
    ...
    // Determine the branch of the student
    const rollNo = student.roll_no;
    // Basic extraction (assuming 21B81A0501 -> branch code is 05)
    let branchCode = null;
    ...
    if (match) branchCode = match[1];
      ...
      // General fallback if pattern is different, e.g. XX...XX (length 10)
      if (rollNo.length === 10) {
    ...
    if (!branchCode) {
       return apiError('Invalid student roll number pattern', 400);
    }
    ...
    if (allowedBranchCodes.length === 0 || !allowedBranchCodes.includes(branchCode)) {
