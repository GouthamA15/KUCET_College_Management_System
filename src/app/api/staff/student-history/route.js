...
import { db } from '@/db';
import { students, studentImportLogs, staffAccounts } from '@/db/schema';
...
export async function GET(req) {
  ...
  try {
    ...
    const addedSub = db.select({
      ...
      staffAccountId: students.added_by_staff_id,
      actionTime: students.created_at,
      totalRecords: sql`NULL`.as('totalRecords')
    })
    .from(students)
    .where(isNotNull(students.added_by_staff_id));

    const updatedSub = db.select({
      rollNo: students.roll_no,
      actionType: sql`'UPDATED'`.as('actionType'),
      staffAccountId: students.updated_by_staff_id,
      actionTime: students.updated_at,
      totalRecords: sql`NULL`.as('totalRecords')
    })
    .from(students)
    .where(and(
      isNotNull(students.updated_by_staff_id),
    ...
    const importedSub = db.select({
      ...
      staffAccountId: studentImportLogs.staff_id,
    ...
    let addedConds = [isNotNull(students.added_by_staff_id)];
    let updatedConds = [
      isNotNull(students.updated_by_staff_id),
      isNotNull(students.updated_at),
      ne(students.updated_at, students.created_at)
