              ...
              >
                <div className="relative w-40 h-40 mb-6">
                  ...
                  <div className="relative w-full h-full rounded-full overflow-hidden border-4 border-white shadow-md group-hover:border-blue-50 transition-colors duration-300">
                    <Image onError={(e) => { e.currentTarget.style.display = 'none'; }} src={dev.image} alt={dev.name} fill sizes="(max-width: 768px) 120px, 160px" className="object-cover" />
                  </div>
                </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2 transform hover:scale-105 active:scale-95 cursor-pointer"
                >
                  <span>🐛</span> Report Bug
                </button>
                <button
                  onClick={() => { setReportType('FEATURE_REQUEST'); setIsModalOpen(true); }}
                  className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2 transform hover:scale-105 active:scale-95 cursor-pointer"
                >
                      ...
                      <div className="p-6">
                        ...
                        {report.screenshot_url && (
                          <div className="relative h-40 w-full rounded-lg overflow-hidden bg-gray-100 cursor-pointer group mb-3" onClick={() => window.open(getAsset(report.screenshot_url), '_blank')}>
                            <Image onError={(e) => { e.currentTarget.style.display = 'none'; }} src={getAsset(report.screenshot_url)} alt="Screenshot" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover group-hover:scale-110 transition-transform duration-500" />
                        ...
                        {report.status === 'OPEN' && isDeveloper && (
                          <button
                            ...
                            className="mt-4 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
                          >
                        ...
                        {report.status === 'RESOLVED' && isDeveloper && (
                          <button
                            ...
                            className="mt-4 w-full bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer"
                          >
                            Reopen
                          </button>
                        )}
            ...
            <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden animate-scale-in">
              ...
              <div className="p-4 bg-gray-50 border-b border-gray-100 flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg font-bold text-sm transition-all cursor-pointer ${reportType === 'BUG' ? 'bg-red-600 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                  🐛 Report Bug
                </button>
                <button
                  onClick={() => setReportType('FEATURE_REQUEST')}
                  className={`flex-1 py-2.5 rounded-lg font-bold text-sm transition-all cursor-pointer ${reportType === 'FEATURE_REQUEST' ? 'bg-purple-600 text-white shadow-md' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                  💡 Suggest Feature
                </button>
              </div>
              ...
              <form onSubmit={handleSubmitReport} className="p-4 sm:p-8">
                  ...
                  {reportType === 'BUG' && (
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-3 rounded-xl border-2 border-gray-100 focus:border-[#0b3578] focus:outline-none transition-colors cursor-pointer"
                      >
                ...
                <div className="mb-8">
                  ...
                  {!screenshotPreview ? (
                    ...
                    <div className="relative h-48 rounded-xl overflow-hidden shadow-md">
                      <Image onError={(e) => { e.currentTarget.style.display = 'none'; }} src={screenshotPreview} alt="Preview" fill sizes="(max-width: 768px) 100vw, 576px" className="object-cover" />
                      <button type="button" onClick={() => { setScreenshot(null); setScreenshotPreview(null); }}
                        className="absolute top-2 right-2 bg-red-600 text-white p-2 rounded-full shadow-lg hover:bg-red-700 transition-colors cursor-pointer">
                ...
                <div className="flex gap-4">
                  <button type="button" onClick={() => setIsModalOpen(false)}
                    className="flex-1 px-6 py-3 border-2 border-gray-100 text-gray-600 font-bold rounded-xl hover:bg-gray-50 transition-colors cursor-pointer">
                    Cancel
                  </button>
                  <button type="submit" disabled={isSubmitting}
                    className={`flex-[2] text-white font-bold py-3 rounded-xl shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer ${reportType === 'FEATURE_REQUEST' ? 'bg-purple-700 hover:bg-purple-800' : 'bg-[#0b3578] hover:bg-[#082a61]'}`}>
