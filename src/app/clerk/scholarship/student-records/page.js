...
function StudentRecordsContent() {
  ...
  const fetchStudent = async (e, overrideMode = null, overrideRoll = null) => {
    ...
    try {
      ...
      if (currentMode === 'roll') {
        res = await fetch(`/api/clerk/scholarship/summary/${encodeURIComponent(currentRoll)}`);
        data = await res.json();
        ...
        const urls = list.map(y => `/api/clerk/scholarship/summary/${encodeURIComponent(currentRoll)}?year=${encodeURIComponent(y)}`);
