...
export default function AssignmentModeSelectorPage() {
  ...
  const handleSelectMode = (mode) => {
    if (mode === 'view') {
      router.push(`/clerk/faculty/attendance/${assignmentId}/history`);
    } else {
      router.push(`/clerk/faculty/attendance/${assignmentId}/take/${mode}`);
    }
  };

  const handleBack = () => {
    router.push('/clerk/faculty/attendance');
  };
  ...
  if (!assignment) {
    return (
      <div className="max-w-7xl mx-auto w-full text-center py-10">
        ...
        <Link href="/clerk/faculty/attendance" className="text-blue-500 hover:underline inline-flex items-center">
