...
function SubjectCard({ assignment, onSelect }) {
...
function AttendanceSubjectList() {
  ...
  const handleSelectAssignment = (assignment) => {
    ...
    router.push(`/clerk/faculty/attendance/${assignment.id}`);
  };
          ...
          {activeAssignments.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activeAssignments.map(assignment => (
                <SubjectCard key={assignment.id} assignment={assignment} onSelect={handleSelectAssignment} />
              ))}
            </div>
          )}
